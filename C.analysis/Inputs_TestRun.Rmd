...
npp_to_plot %>%  
  ggplot(aes(ymd(datetime), value)) + 
  geom_point() + 
  THEME + 
  labs(title = 'Average Monthly NPP',
       subtitle = 'Cohort (Species x DBH)',
       y = unique(npp_to_plot$unit)) + 
  theme(legend.position = 'none') + 
  geom_vline(xintercept = date('1979-01-01'), color = 'red') + 
  geom_vline(xintercept = date('2015-01-01'), color = 'blue') 
...
My first response to this plot was what is up with the negative values? 


```{r}
# When is NPP negative? 
negative_NPP <- npp_to_plot[MMEAN_NPP_CO < 0 & year > 2010]
unique(negative_NPP$month)
```

Looks like NPP is negative during the winter months, which make sense. 
...
```{r}
out %>%  
  ggplot(aes(datetime, MMEAN_NEP_PY)) + 
  geom_point() + 
  THEME + 
  labs(title = 'Average Monthly NEP',
       subtitle = 'Stand  NEP',
       y = unique(out$unit)) + 
  theme(legend.position = 'none') + 
  geom_vline(xintercept = date('1979-01-01'), color = 'red') + 
...
```{r}
out[month == 8] %>%  #  subset for the mid summer value
  ggplot(aes(year, MMEAN_NEP_PY)) + 
  geom_line() + 
  THEME + 
  labs(title = 'Average Monthly NEP',
       subtitle = 'Stand  NEP, Mid Summer',
       y = unique(out$unit), 
       x = 'Mid Summer Value') + 
  theme(legend.position = 'none') + 
  geom_vline(xintercept = 1979, color = 'red') + 
...
Interesting what year has a NEP value less than 0? 

```{r}
out[month == 8 &  MMEAN_NEP_PY < 0][, list(MMEAN_NEP_PY, unit,year, month, description)]
```


So it looks like August in 1939 NEP is very low, I wonder why that is? Does it concern us that this happens when we are looping through the climate data? 
...
* Do we want to take a look at the mid summer values? Or the annual values? 
* Should we be concerned about about the negative NEP in august? 
