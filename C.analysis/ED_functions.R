...
monthly_NPP <- function(object, scn){
  ...
  out$unit <- gsub(pattern = '/yr', replacement = "", x = out$unit)
...
get_monthly_data  <- function(path, scn){
  ...
  object <- readRDS(path)
    ...
    data <- rbind(monthly_NPP(object, scn), 
                  ...
                  monthly_GPP(object, scn), fill = TRUE)
...
calculate_annual_values <- function(dt){
  ...
  req_vars <- c('NPP_PFT_CO', 'NEP_patch', 'GPP_PFT_CO', 'AGB_PFT_CO')
  ...
  annual_avg$description <- paste('Annual Mean',  annual_avg$description, sep = ' ')
