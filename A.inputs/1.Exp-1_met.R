...
# Define directories. 
BASE_DIR  <- here::here()
WRITE_TO  <- file.path(BASE_DIR, 'exp-1_met2'); dir.create(WRITE_TO, showWarnings = FALSE, recursive = TRUE) # This is the directory to write where the different cases should be set up. 
...
# TODO I had to manuual change the ED met dirver pointer, this shoudl be something that can be changed with a point in a direction! 
# Save all of the ED information as a single data frame. 
case <- data.frame(casename = cases, IYEARA = IYEARA, IYEARZ = IYEARZ,
                   EVENT_FILE = event_file, 
                   ED_MET_DRIVER_DB = file.path(INPUT_DIR, "NARR-ED2_met3", "ED_MET_DRIVER_HEADER"))
