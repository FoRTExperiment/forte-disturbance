...
setup_ed_run <- function(case, input_dir = INPUT_DIR, write_to = WRITE_TO){
  ...
  unrecognized  <- !names(case) %in% c('casename', potential_ed_settings, 'data')
  ...
  OUTOUT <- file.path(write_to, case[['casename']])
  ...
  if(any('data'%in% names(case))){
    d <- case[['data']][[1]]
    unrecognized <- !names(d) %in% potential_ed_params
    assert_that(sum(unrecognized) == 0, msg = paste0(paste(names(d)[unrecognized], collapse = ', '), ' unrecognized pft params in the nested data column.'))
    
    # Only if there is paramter information being passed in with the cases. 
    config_xml <- as_pft_list(d) %>% fortebaseline::write_ed2_xml()
  ...
  from_case <- case[!names(case) %in% c('casename', 'data')]
  if(ncol(from_case) > 0){
